...
class DesaInventarisTest extends TestCase
{
    ...
    public function admin_desa_dapat_melihat_daftar_inventaris_di_desanya_sendiri()
    {
        ...
        $response->assertInertia(function (AssertableInertia $page): void {
            $page
                ->component('Desa/Inventaris/Index')
    ...
    #[Test]
    public function daftar_inventaris_desa_mendukung_pagination_dan_tetap_scoped()
    {
        $adminDesa = User::factory()->create([
            'area_id' => $this->desaA->id,
            'scope' => 'desa',
        ...
        $response = $this->actingAs($adminDesa)->get('/desa/inventaris?page=2&per_page=10');
        ...
        $response->assertInertia(function (AssertableInertia $page): void {
            $page
                ->component('Desa/Inventaris/Index')
                ->has('inventaris.data', 2)
                ->where('inventaris.current_page', 2)
                ->where('inventaris.per_page', 10)
                ->where('inventaris.total', 12)
                ->where('filters.per_page', 10);
        });
    }

    #[Test]
    public function per_page_tidak_valid_di_inventaris_desa_kembali_ke_default()
    {
        $adminDesa = User::factory()->create([
            'area_id' => $this->desaA->id,
            'scope' => 'desa',
        ...
        $response = $this->actingAs($adminDesa)->get('/desa/inventaris?per_page=999');
