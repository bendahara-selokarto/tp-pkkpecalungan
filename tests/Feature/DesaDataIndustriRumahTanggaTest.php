...
class DesaDataIndustriRumahTanggaTest extends TestCase
{
    ...
    #[Test]
    public function admin_desa_hanya_melihat_data_industri_rumah_tangga_pada_tahun_anggaran_aktif(): void
    {
        $adminDesa = User::factory()->create([
            'area_id' => $this->desaA->id,
            'scope' => 'desa',
        ...
        $response->assertOk();
        $response->assertDontSee('Lele Lama');
        $response->assertInertia(function (AssertableInertia $page): void {
            $page
                ->component('Desa/DataIndustriRumahTangga/Index')
                ->has('dataIndustriRumahTanggaItems.data', 1)
