...
| `A2` | `scope=kecamatan` tetapi `area_id` mengarah ke area level `desa` | role `admin-kecamatan`, scope `kecamatan`, area desa | request route `kecamatan.*.report` ditolak `403` atau scope dinetralkan (no access) | `tests/Feature/AgendaSuratReportPrintTest.php` (`ekspedisi.report`), `tests/Feature/DashboardActivityChartTest.php` (`role_kecamatan_tetapi_area_level_desa`) |
