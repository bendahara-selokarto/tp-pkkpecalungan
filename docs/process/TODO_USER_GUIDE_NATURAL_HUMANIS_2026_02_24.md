...
- [ ] `U3` Kunci glossary istilah user-facing di `docs/domain/TERMINOLOGY_NORMALIZATION_MAP.md` agar bahasa panduan konsisten.
- [x] `U4` Buat skeleton dokumen di `docs/user-guide/` sesuai struktur arsitektur di atas.
- [ ] `U5` Tulis konten fase 1 (mulai cepat + peran sekretaris + FAQ umum).
- [ ] `U6` Tulis konten fase 2 (peran pokja + super-admin + alur dashboard/filter/laporan).
- [ ] `U7` Jalankan copywriting pass: audit bahasa teknis, konsistensi istilah, dan kejelasan instruksi tindakan.
- [ ] `U8` Lakukan review lintas peran (minimal 1 skenario sukses + 1 skenario kendala per peran).
- [ ] `U9` Finalisasi indeks panduan dan tautan antarhalaman agar navigasi satu arah dan tidak membingungkan.
...
## Acceptance Criteria
- [ ] Pengguna baru bisa menyelesaikan 3 tugas inti tanpa pendamping:
  - login dan memahami halaman kerja,
  - input/update data sesuai peran,
  - membaca dashboard dan mengekspor/cetak laporan.
- [ ] Tidak ada label teknis internal pada judul/subjudul/CTA user guide.
- [ ] Setiap halaman maksimal fokus pada satu tujuan utama.
- [ ] Semua langkah instruksi dapat dipetakan ke perilaku UI/route yang benar.
- [x] Minimal satu dokumen panduan siap cetak tersedia untuk uji distribusi.

## Validasi
- [ ] Review konten oleh tim domain: istilah sesuai pedoman domain utama.
- [ ] Review konten oleh tim produk/UI: bahasa natural dan tidak ambigu.
- [ ] Smoke test manual: ikuti panduan langkah demi langkah pada akun role berbeda dan pastikan hasilnya sesuai.
- [ ] Konsistensi lintas dokumen: terminologi user guide sinkron dengan `TERMINOLOGY_NORMALIZATION_MAP`.
