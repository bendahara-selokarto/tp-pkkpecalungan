...
| 4.16c | `catatan-data-kegiatan-warga-pkk-rw` (reference) | Catatan Data dan Kegiatan Warga Kelompok PKK RW | Struktur autentik 32 kolom sudah terverifikasi visual sampai level merge (`rowspan`/`colspan`), referensi mapping: `docs/domain/CATATAN_PKK_RW_4_16C_MAPPING.md` | belum ada view report khusus | Dokumen autentik `d:\\pedoman\\183.pdf` (Lampiran 4.16c) | reference-verified (siap sinkronisasi kontrak, implementasi report belum dijalankan) |
