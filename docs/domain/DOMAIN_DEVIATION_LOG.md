...
| `DV-006` | 2026-02-22 | 4.16d / `rekap-catatan-data-kegiatan-warga-rw` | Label canonical sementara | Struktur header 33 kolom tervalidasi visual dari dokumen autentik `185.pdf`, namun token judul halaman penuh belum terbaca di environment saat implementasi. | Parser text-layer lokal belum tersedia saat fase implementasi; kebutuhan report 4.16d tetap diprioritaskan agar flow end-to-end aktif. | Potensi perbedaan minor label judul PDF jika dokumen penuh menunjukkan frasa final yang berbeda. | Jalankan konfirmasi token identitas judul dari halaman penuh 4.16d; jika berbeda, update judul PDF + terminology + matrix + fixture/test terkait pada concern terpisah. | `accepted` |
...
| `DV-011` | 2026-02-22 | 4.18a-4.18c / `rekap-ibu-hamil-melahirkan-*` | Sumber field maternal/kelahiran/kematian | Lampiran autentik 4.18a-4.18c membutuhkan indikator eksplisit (hamil/melahirkan/nifas, kematian ibu/bayi/balita, akte kelahiran), namun kontrak input saat ini belum memiliki field dedicated untuk seluruh indikator tersebut. | Struktur operasional `data_wargas` dan `data_warga_anggotas` masih general-purpose, sehingga status maternal/kematian diturunkan dari `keterangan` dan atribut umur/jenis kelamin. | Risiko undercount/overcount pada indikator 4.18 saat data `keterangan` tidak baku atau tidak lengkap. | Pertahankan report 4.18 sebagai proyeksi operasional (report-only), dokumentasikan keterbatasan pada mapping domain, dan evaluasi concern migrasi terpisah untuk field dedicated indikator maternal/kelahiran/kematian/akte. | `accepted` |
