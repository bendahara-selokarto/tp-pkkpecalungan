...
| 4.21 | `data-kegiatan-pkk-pokja-i` | `resources/views/pdf/data_kegiatan_pkk_pokja_i_report.blade.php` | Header merge 27 kolom dengan grup puncak `PENGHAYATAN DAN PENGAMALAN PANCASILA DAN GOTONG ROYONG` + subgrup `KISAH/KRISAN/KILAS/KTIAT/KISAK/PKBN` | Agregasi lintas `anggota-pokja` + `activities`, fallback nilai `0` untuk `metode` dan `sasaran` | `landscape` | level + printedBy + printedAt | `pass` |
