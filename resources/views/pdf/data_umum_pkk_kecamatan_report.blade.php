...
<head>
    <meta charset="UTF-8">
    <title>Data Umum PKK Kecamatan</title>
    <style>
...
<body>
    @php
        ...
        $metaData = $meta ?? [];
        $totalsData = $totals ?? [];
        $tpPkkKecamatanData = $tpPkkKecamatanTotals ?? [];
    @endphp

    <div class="lampiran">LAMPIRAN 4.20b</div>
    ...
    <table class="main-table">
        <colgroup>
            ...
            <col style="width: 130px;"> <!-- DESA -->
            @for ($i = 0; $i < 18; $i++)
                <col style="width: 32px;"> <!-- 3-20 -->
            @endfor
        ...
        <tbody>
            @forelse ($items as $item)
                <tr>
                    ...
                    <td class="left">{{ $item['nama_desa_kelurahan'] ?? '-' }}</td>
                    <td class="center">{{ $item['jumlah_dusun_lingkungan'] ?? 0 }}</td>
            ...
            @empty
                <tr>
                    <td colspan="21" class="center">Data umum PKK tingkat kecamatan belum tersedia.</td>
                </tr>
            @endforelse
            <tr>
                <td></td>
                <td class="left"><strong>TP PKK KECAMATAN</strong></td>
                <td colspan="8"></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_kader_anggota_tp_pkk_l'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_kader_anggota_tp_pkk_p'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_kader_umum_l'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_kader_umum_p'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_kader_khusus_l'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_kader_khusus_p'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_tenaga_sekretariat_honorer_l'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_tenaga_sekretariat_honorer_p'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_tenaga_sekretariat_bantuan_l'] ?? 0 }}</strong></td>
                <td class="center"><strong>{{ $tpPkkKecamatanData['jumlah_tenaga_sekretariat_bantuan_p'] ?? 0 }}</strong></td>
                <td></td>
            </tr>
            <tr>
                <td></td>
                <td class="left"><strong>JUMLAH</strong></td>
                <td class="center"><strong>{{ $totalsData['jumlah_dusun_lingkungan'] ?? 0 }}</strong></td>
