...
import { mdiAccountCheckOutline, mdiPlus, mdiEye, mdiPencil, mdiTrashCan, mdiFileDownloadOutline } from '@mdi/js'
...
const deleteConfirmationMessage = 'Apakah Anda yakin ingin menghapus data buku daftar hadir simulasi ini?'

const moduleMode = computed(() => page.props.auth?.user?.moduleModes?.['buku-daftar-hadir-simulasi'])
...
const confirmDelete = () => {
  if (itemToDelete.value) {
    router.delete(`/desa/simulasi/buku-daftar-hadir/${itemToDelete.value.id}`, {
      onSuccess: () => {
        isDeleteModalActive.value = false
        itemToDelete.value = null
      },
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountCheckOutline" title="Buku Daftar Hadir Simulasi" main />

    <CardBox class="mb-6" has-table>
      <div class="p-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between gap-4">
        <div>
          <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daftar Hadir Simulasi</h3>
          <p class="text-sm text-gray-500">Tahun Anggaran: {{ filters.tahun_anggaran }}</p>
        </div>
        <div class="flex items-center gap-2">
          <Link
            v-if="canCreate"
            href="/desa/simulasi/buku-daftar-hadir/create"
              ...
              <td class="px-3 py-3">
                <BaseButtons type="justify-center" no-wrap>
                  <BaseButton
                    ...
                    :icon="mdiEye"
                    small
                    :href="`/desa/simulasi/buku-daftar-hadir/${item.id}`"
                    title="Detail"
                  />
                  <BaseButton
                    v-if="canUpdate"
                    ...
                    :icon="mdiPencil"
                    small
                    :href="`/desa/simulasi/buku-daftar-hadir/${item.id}/edit`"
                    title="Edit"
                  />
                  <BaseButton
                    v-if="canDelete"
