...
const form = useForm({
  tingkat: props.praKoperasi.tingkat ?? tingkatOptions[0]?.value ?? '',
  jumlah_kelompok: props.praKoperasi.jumlah_kelompok ?? '',
  jumlah_peserta: props.praKoperasi.jumlah_peserta ?? '',
  keterangan: props.praKoperasi.keterangan ?? '',
})

const submit = () => {
  form.put(`/desa/pra-koperasi-up2k/${props.praKoperasi.id}`)
}
</script>

<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountGroup" title="Edit Pra Koperasi/UP2K Desa" main />
