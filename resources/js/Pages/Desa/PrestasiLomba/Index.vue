...
const hapusPrestasiLomba = (id) => {
  if (!window.confirm('Apakah Anda yakin ingin menghapus data prestasi lomba ini?')) {
    return
  }

  router.delete(`/desa/prestasi-lomba/${id}`)
}
</script>

<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiTrophy" title="Prestasi Lomba Desa" main />
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-3 py-3">
                <div class="flex items-center gap-2">
                  <Link
                    :href="`/desa/prestasi-lomba/${item.id}`"
                  ...
                  <Link
                    :href="`/desa/prestasi-lomba/${item.id}/edit`"
