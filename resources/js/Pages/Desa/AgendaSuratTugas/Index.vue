...
import { mdiCardAccountDetailsOutline, mdiPlus, mdiEye, mdiPencil, mdiTrashCan, mdiFileDownloadOutline, mdiPrinter } from '@mdi/js'
...
const deleteConfirmationMessage = 'Apakah Anda yakin ingin menghapus data agenda surat tugas ini?'
...
const confirmDelete = () => {
  if (itemToDelete.value) {
    router.delete(`/desa/agenda-surat-tugas/${itemToDelete.value.id}`, {
      onSuccess: () => {
        isDeleteModalActive.value = false
        itemToDelete.value = null
      },
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiCardAccountDetailsOutline" title="Agenda Surat Tugas" main />

    <CardBox class="mb-6" has-table>
      <div class="p-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between gap-4">
        <div>
          <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daftar Agenda Surat Tugas</h3>
          <p class="text-sm text-gray-500">Tahun Anggaran: {{ filters.tahun_anggaran }}</p>
        </div>
        <div class="flex items-center gap-2">
          <a
            v-if="canPrint"
            href="/desa/agenda-surat-tugas/report/pdf"
            target="_blank"
            rel="noopener"
          ...
          <Link
            v-if="canCreate"
            href="/desa/agenda-surat-tugas/create"
        ...
        <table class="w-full min-w-[1000px] text-sm text-left">
          <thead class="border-b border-gray-200 dark:border-slate-700">
            <tr class="text-left text-gray-600 dark:text-gray-300">
              <th class="px-3 py-3 font-semibold">Nomor & Kode Surat</th>
          ...
          <tbody>
            ...
            >
              <td class="px-3 py-3 text-gray-900 font-medium dark:text-gray-100">{{ item.nomor_surat }}</td>
              ...
              <td class="px-3 py-3 text-gray-700 dark:text-gray-300">{{ item.lampiran || '-' }}</td>
              ...
              <td class="px-3 py-3">
                <BaseButtons type="justify-center" no-wrap>
                  <BaseButton
                    ...
                    :icon="mdiEye"
                    small
                    :href="`/desa/agenda-surat-tugas/${item.id}`"
                    title="Detail"
                  />
                  <BaseButton
                    v-if="canUpdate"
                    ...
                    :icon="mdiPencil"
                    small
                    :href="`/desa/agenda-surat-tugas/${item.id}/edit`"
                    title="Edit"
                  />
                  <BaseButton
                    v-if="canDelete"
            ...
            <tr v-if="items.data.length === 0">
              <td colspan="8" class="px-6 py-12 text-center text-gray-500">
                Data agenda surat tugas belum tersedia.
              </td>
            </tr>
          </tbody>
        </table>
