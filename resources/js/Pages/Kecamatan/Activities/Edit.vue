...
const submit = () => {
  form.put(`/kecamatan/activities/${props.activity.id}`)
}
</script>

<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiClipboardList" title="Edit Kegiatan Kecamatan" main />
      ...
      <form class="space-y-5" @submit.prevent="submit">
        ...
        <div class="grid gap-5 md:grid-cols-2">
          <div>
            ...
            <input v-model="form.activity_date" type="text" inputmode="numeric" placeholder="DD/MM/YYYY" pattern="^\d{2}/\d{2}/\d{4}$" class="w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" required>
