...
const updatePerPage = (event) => {
  ...
  router.get('/kecamatan/literasi-warga', {
    ...props.filters,
    page: 1,
...
const deleteConfirmationMessage = 'Apakah Anda yakin ingin menghapus data literasi warga ini?'
const isDeleteModalActive = ref(false)
const deletingId = ref(null)

const hapusLiterasiWarga = (id) => {
  deletingId.value = id
...
const confirmDelete = () => {
  ...
  router.delete(`/kecamatan/literasi-warga/${deletingId.value}`, {
    onFinish: () => {
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiBookOpenVariant" title="Data Literasi Warga Kecamatan" main />

    <CardBox>
      <div class="mb-4 flex items-center justify-between gap-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daftar Data Literasi Warga</h3>
        <Link
          href="/kecamatan/literasi-warga/create"
        ...
        <table class="w-full min-w-[680px] text-sm">
          <thead class="border-b border-gray-200 dark:border-slate-700">
            <tr class="text-left text-gray-600 dark:text-gray-300">
              ...
              <th class="px-3 py-3 font-semibold text-center">Jumlah Warga 3 (Tiga) Buta</th>
          ...
          <tbody>
            <tr
              v-for="(item, index) in props.literasiWargaItems.data"
              :key="item.id"
              class="border-b border-gray-100 align-top dark:border-slate-800"
            >
              <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-300">{{ index + 1 }}</td>
              <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-300">{{ item.jumlah_tiga_buta }}</td>
              ...
              <td class="px-3 py-3">
                <div class="flex items-center gap-2">
                  <Link
                    :href="`/kecamatan/literasi-warga/${item.id}`"
                  ...
                  <Link
                    :href="`/kecamatan/literasi-warga/${item.id}/edit`"
                  ...
                  <button
                    ...
                    @click="hapusLiterasiWarga(item.id)"
                  >
                    Hapus
                  </button>
                </div>
      ...
      <PaginationBar
        :links="props.literasiWargaItems.links"
        :from="props.literasiWargaItems.from"
        :to="props.literasiWargaItems.to"
        :total="props.literasiWargaItems.total"
      />
    </CardBox>
