...
const form = useForm({
  indikator: props.indicatorOptions[0]?.value ?? '',
  jumlah: 0,
  keterangan: '',
})

const submit = () => {
  form.post('/kecamatan/paar')
}
</script>

<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountGroup" title="Tambah Data PAAR" main />
    ...
    <CardBox class="max-w-4xl">
      <form class="space-y-5" @submit.prevent="submit">
        <div>
          ...
          <select v-model="form.indikator" class="w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" required>
            <option v-for="option in indicatorOptions" :key="option.value" :value="option.value">
              {{ option.label }}
            </option>
          </select>
          <p v-if="form.errors.indikator" class="mt-1 text-xs text-rose-600">{{ form.errors.indikator }}</p>
        </div>
