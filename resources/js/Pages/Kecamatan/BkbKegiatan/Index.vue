...
const updatePerPage = (event) => {
  ...
  router.get('/kecamatan/bkb-kegiatan', {
    ...props.filters,
    page: 1,
...
const deleteConfirmationMessage = 'Apakah Anda yakin ingin menghapus data BKB ini?'
...
const confirmDelete = () => {
  ...
  router.delete(`/kecamatan/bkb-kegiatan/${deletingId.value}`, {
    onFinish: () => {
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountGroup" title="Data Kegiatan BKB Kecamatan" main />

    <CardBox>
      <div class="mb-4 flex items-center justify-between gap-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daftar Data Kegiatan BKB</h3>
        <Link
          href="/kecamatan/bkb-kegiatan/create"
        ...
        <table class="w-full min-w-[860px] text-sm">
          <thead class="border-b border-gray-200 dark:border-slate-700">
            <tr class="text-left text-gray-600 dark:text-gray-300">
              ...
              <th class="px-3 py-3 font-semibold text-center">Jumlah Ibu Peserta</th>
              <th class="px-3 py-3 font-semibold text-center">Jumlah APE (Set)</th>
              <th class="px-3 py-3 font-semibold text-center">Jumlah Kelompok Simulasi</th>
          ...
          <tbody>
            <tr
              v-for="(item, index) in props.bkbKegiatanItems.data"
              :key="item.id"
            ...
            >
              ...
              <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-300">{{ item.jumlah_ibu_peserta }}</td>
              <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-300">{{ item.jumlah_ape_set }}</td>
              <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-300">{{ item.jumlah_kelompok_simulasi }}</td>
              ...
              <td class="px-3 py-3">
                <div class="flex items-center gap-2">
                  <Link
                    :href="`/kecamatan/bkb-kegiatan/${item.id}`"
                  ...
                  <Link
                    :href="`/kecamatan/bkb-kegiatan/${item.id}/edit`"
      ...
      <PaginationBar
        :links="props.bkbKegiatanItems.links"
        :from="props.bkbKegiatanItems.from"
        :to="props.bkbKegiatanItems.to"
        :total="props.bkbKegiatanItems.total"
      />
    </CardBox>
