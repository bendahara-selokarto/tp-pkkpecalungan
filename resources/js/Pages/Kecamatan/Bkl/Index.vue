...
const confirmDelete = () => {
  ...
  router.delete(`/kecamatan/bkl/${deletingId.value}`, {
    onFinish: () => {
...
const updatePerPage = (event) => {
  ...
  router.get('/kecamatan/bkl', { per_page: selectedPerPage }, {
    preserveScroll: true,
    preserveState: true,
    replace: true,
  })
...
<template>
  <SectionMain>
    ...
    <CardBox>
        ...
        <table class="w-full min-w-[1120px] text-sm">
          <thead class="border-b border-gray-200 dark:border-slate-700">
            <tr class="text-left text-gray-600 dark:text-gray-300">
              ...
              <th class="px-3 py-3 font-semibold">Nama BKL</th>
          ...
          <tbody>
            <tr
              v-for="item in bklItems.data"
              :key="item.id"
              class="border-b border-gray-100 align-top dark:border-slate-800"
            >
              <td class="px-3 py-3 text-gray-900 dark:text-gray-100">{{ item.desa }}</td>
              <td class="px-3 py-3 text-gray-700 dark:text-gray-300">{{ item.nama_bkl }}</td>
              ...
              <td class="px-3 py-3">
                <div class="flex items-center gap-2">
                  ...
                  <Link
                    :href="`/kecamatan/bkl/${item.id}/edit`"
            ...
            <tr v-if="bklItems.data.length === 0">
      ...
      <PaginationBar
        :links="bklItems.links"
        :from="bklItems.from"
        :to="bklItems.to"
        :total="bklItems.total"
      />
    </CardBox>
