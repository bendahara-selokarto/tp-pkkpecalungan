...
const hapusAnggotaTimPenggerak = (id) => {
  if (!window.confirm('Apakah Anda yakin ingin menghapus data anggota tim penggerak ini?')) {
    return
  }

  router.delete(`/kecamatan/anggota-tim-penggerak/${id}`)
}
</script>

<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountGroup" title="Anggota Tim Penggerak Kecamatan" main />
