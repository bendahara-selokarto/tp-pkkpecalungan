...
const deleteConfirmationMessage = 'Apakah Anda yakin ingin menghapus data keluarga ini?'
...
const updatePerPage = (event) => {
  ...
  router.get('/kecamatan/data-keluarga', { per_page: selectedPerPage }, {
    preserveScroll: true,
    preserveState: true,
    replace: true,
  })
...
const confirmDelete = () => {
  ...
  router.delete(`/kecamatan/data-keluarga/${deletingId.value}`, {
    onFinish: () => {
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountGroup" title="Data Keluarga Kecamatan" main />

    <CardBox>
      <div class="mb-4 flex items-center justify-between gap-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daftar Data Keluarga</h3>
        <div class="flex items-center gap-2">
          ...
          <a
            href="/kecamatan/data-keluarga/report/pdf"
            target="_blank"
            rel="noopener"
          ...
          <Link
            href="/kecamatan/data-keluarga/create"
        ...
        <table class="w-full min-w-[900px] text-sm">
          <thead class="border-b border-gray-200 dark:border-slate-700">
            <tr class="text-left text-gray-600 dark:text-gray-300">
              <th class="px-3 py-3 font-semibold">Kategori Keluarga</th>
              <th class="px-3 py-3 font-semibold text-center">Jumlah Keluarga</th>
          ...
          <tbody>
            <tr
              v-for="item in props.dataKeluargaItems.data"
              :key="item.id"
              class="border-b border-gray-100 align-top dark:border-slate-800"
            >
              <td class="px-3 py-3 text-gray-900 dark:text-gray-100">{{ item.kategori_keluarga }}</td>
              <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-300">{{ item.jumlah_keluarga }}</td>
              <td class="px-3 py-3 text-gray-700 dark:text-gray-300">{{ item.keterangan || '-' }}</td>
              <td class="px-3 py-3">
                <div class="flex items-center gap-2">
                  <Link
                    :href="`/kecamatan/data-keluarga/${item.id}`"
                  ...
                  <Link
                    :href="`/kecamatan/data-keluarga/${item.id}/edit`"
                  ...
                  <button
                    ...
                    @click="hapusDataKeluarga(item.id)"
                  >
                    Hapus
                  </button>
                </div>
            ...
            <tr v-if="props.dataKeluargaItems.data.length === 0">
      ...
      <PaginationBar
        :links="dataKeluargaItems.links"
        :from="dataKeluargaItems.from"
        :to="dataKeluargaItems.to"
        :total="dataKeluargaItems.total"
      />
    </CardBox>
