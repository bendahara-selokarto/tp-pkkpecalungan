...
const deleteConfirmationMessage = 'Apakah Anda yakin ingin menghapus data daftar hadir ini?'
...
const updatePerPage = (event) => {
  ...
  router.get('/kecamatan/buku-daftar-hadir', { per_page: selectedPerPage }, {
    preserveScroll: true,
    preserveState: true,
    replace: true,
  })
...
const confirmDelete = () => {
  ...
  router.delete(`/kecamatan/buku-daftar-hadir/${deletingId.value}`, {
    onFinish: () => {
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiNotebookEditOutline" title="Buku Daftar Hadir Kecamatan" main />

    <CardBox>
      <div class="mb-4 flex items-center justify-between gap-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daftar Kehadiran</h3>
        <div class="flex items-center gap-2">
          ...
          <a
            href="/kecamatan/buku-daftar-hadir/report/pdf"
            target="_blank"
            rel="noopener"
          ...
          <Link
            href="/kecamatan/buku-daftar-hadir/create"
        ...
        <table class="w-full min-w-[760px] text-sm">
          <thead class="border-b border-gray-200 dark:border-slate-700">
            <tr class="text-left text-gray-600 dark:text-gray-300">
              <th class="px-3 py-3 font-semibold">Tanggal Hadir</th>
          ...
          <tbody>
            ...
            >
              <td class="px-3 py-3 text-gray-700 dark:text-gray-300">{{ formatDate(item.attendance_date) }}</td>
              <td class="px-3 py-3 text-gray-900 dark:text-gray-100">{{ item.activity_title || '-' }}</td>
              ...
              <td class="px-3 py-3">
                <div class="flex items-center gap-2">
                  <Link
                    :href="`/kecamatan/buku-daftar-hadir/${item.id}`"
                  ...
                  <Link
                    :href="`/kecamatan/buku-daftar-hadir/${item.id}/edit`"
            ...
            <tr v-if="items.data.length === 0">
              <td colspan="5" class="px-3 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
                Data daftar hadir belum tersedia.
              </td>
            </tr>
          </tbody>
        </table>
