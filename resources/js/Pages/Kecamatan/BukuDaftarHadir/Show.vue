...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountMultipleCheckOutline" title="Detail Buku Daftar Hadir" main />

    <CardBox class="max-w-3xl space-y-4">
      <div>
        <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Judul Daftar Hadir</p>
        <p class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ props.item.title }}</p>
      </div>

      <div v-if="props.item.file_url">
        <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Berkas Daftar Hadir</p>
        <a
      ...
      <div class="flex items-center justify-end gap-2">
        ...
        <Link
          :href="`/kecamatan/buku-daftar-hadir/${props.item.id}/edit`"
