...
const deleteConfirmationMessage = 'Apakah Anda yakin ingin menghapus data buku ekspedisi ini?'
...
const updatePerPage = (event) => {
  ...
  router.get('/kecamatan/buku-ekspedisi', { per_page: selectedPerPage }, {
    preserveScroll: true,
    preserveState: true,
    replace: true,
  })
...
const confirmDelete = () => {
  ...
  router.delete(`/kecamatan/buku-ekspedisi/${deletingId.value}`, {
    onFinish: () => {
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiNotebookEditOutline" title="Buku Ekspedisi Kecamatan" main />

    <CardBox>
      <div class="mb-4 flex items-center justify-between gap-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daftar Berkas Ekspedisi</h3>
        <div class="flex items-center gap-2">
          ...
          <Link
            href="/kecamatan/buku-ekspedisi/create"
        ...
        <table class="w-full min-w-[760px] text-sm">
          <thead class="border-b border-gray-200 dark:border-slate-700">
            <tr class="text-left text-gray-600 dark:text-gray-300">
              <th class="px-3 py-3 font-semibold">Judul/Keterangan</th>
              <th class="px-3 py-3 font-semibold">Nama File Asli</th>
              <th class="px-3 py-3 font-semibold">Ukuran</th>
              <th class="px-3 py-3 font-semibold">Tanggal Unggah</th>
          ...
          <tbody>
            ...
            >
              <td class="px-3 py-3 text-gray-900 dark:text-gray-100 font-medium">{{ item.title }}</td>
              <td class="px-3 py-3 text-gray-700 dark:text-gray-300">{{ item.original_name }}</td>
              <td class="px-3 py-3 text-gray-700 dark:text-gray-300">{{ formatBytes(item.size_bytes) }}</td>
              <td class="px-3 py-3 text-gray-700 dark:text-gray-300">{{ item.created_at }}</td>
              <td class="px-3 py-3">
                <div class="flex items-center gap-2">
                  <a
                    :href="`/kecamatan/buku-ekspedisi/${item.id}/download`"
                    class="inline-flex rounded-md border border-emerald-200 px-3 py-1.5 text-xs font-semibold text-emerald-700 hover:bg-emerald-50 dark:border-emerald-900/50 dark:text-emerald-300 dark:hover:bg-emerald-900/20"
                  >
                    Unduh
                  </a>
                  <Link
                    :href="`/kecamatan/buku-ekspedisi/${item.id}/edit`"
            ...
            <tr v-if="items.data.length === 0">
              <td colspan="5" class="px-3 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
                Data buku ekspedisi belum tersedia.
              </td>
            </tr>
          </tbody>
        </table>
