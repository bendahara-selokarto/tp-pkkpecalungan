...
const form = useForm({
  name: props.inventaris.name ?? '',
  asal_barang: props.inventaris.asal_barang ?? '',
  tanggal_penerimaan: props.inventaris.tanggal_penerimaan ?? '',
  tempat_penyimpanan: props.inventaris.tempat_penyimpanan ?? '',
  keterangan: props.inventaris.keterangan ?? props.inventaris.description ?? '',
  quantity: props.inventaris.quantity ?? 1,
  unit: props.inventaris.unit ?? 'unit',
  condition: props.inventaris.condition ?? 'baik',
})
...
const submit = () => {
  form
    ...
    .put(`/kecamatan/inventaris/${props.inventaris.id}`)
}
</script>

<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiPackageVariantClosed" title="Edit Buku Inventaris Kecamatan" main />
