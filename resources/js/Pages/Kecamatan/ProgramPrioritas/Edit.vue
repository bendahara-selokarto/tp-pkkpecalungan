...
const JADWAL_BULAN_OPTIONS = Array.from({ length: 12 }, (_, index) => {
  const month = index + 1
  return { key: `jadwal_bulan_${month}`, label: String(month), month }
})

const hasMonthlyJadwalPayload = JADWAL_BULAN_OPTIONS.some(({ key }) => Boolean(props.programPrioritas[key]))

const resolveInitialMonthlyJadwal = (month) => {
  if (hasMonthlyJadwalPayload) {
    return Boolean(props.programPrioritas[`jadwal_bulan_${month}`])
  }

  if (month <= 3) return Boolean(props.programPrioritas.jadwal_i)
  if (month <= 6) return Boolean(props.programPrioritas.jadwal_ii)
  if (month <= 9) return Boolean(props.programPrioritas.jadwal_iii)
  return Boolean(props.programPrioritas.jadwal_iv)
}

const form = useForm({
  program: props.programPrioritas.program ?? '',
  prioritas_program: props.programPrioritas.prioritas_program ?? '',
  kegiatan: props.programPrioritas.kegiatan ?? '',
  sasaran_target: props.programPrioritas.sasaran_target ?? '',
  ...Object.fromEntries(
    JADWAL_BULAN_OPTIONS.map(({ key, month }) => [key, resolveInitialMonthlyJadwal(month)]),
  ),
  sumber_dana_pusat: props.programPrioritas.sumber_dana_pusat ?? false,
  sumber_dana_apbd: props.programPrioritas.sumber_dana_apbd ?? false,
...
const submit = () => {
  form.put(`/kecamatan/program-prioritas/${props.programPrioritas.id}`)
}
</script>

<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiClipboardList" title="Edit Program Prioritas Kecamatan" main />
