...
const form = useForm({
  _method: 'PUT',
  title: props.fotoKegiatan.title,
  activity_date: props.fotoKegiatan.activity_date,
  description: props.fotoKegiatan.description || '',
  image_upload: null,
})
...
const submit = () => {
  form.post(`/kecamatan/foto-kegiatan/${props.fotoKegiatan.id}`, {
    forceFormData: true,
  })
}
</script>
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiCamera" title="Edit Foto Kegiatan" main />
    ...
    <CardBox class="max-w-2xl">
      <form class="space-y-5" @submit.prevent="submit">
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Ganti Foto (Opsional)</label>
          <div v-if="fotoKegiatan.image_url" class="mb-2">
            <img :src="fotoKegiatan.image_url" class="h-32 w-48 rounded object-cover shadow-sm" alt="Current Photo">
        ...
        <div class="flex items-center justify-end gap-2">
          <Link href="/kecamatan/foto-kegiatan" class="inline-flex rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
            Batal
          </Link>
