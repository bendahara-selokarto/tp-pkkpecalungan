...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiCalendarTextOutline" title="Detail Notulen Simulasi" main>
      <Link
    ...
    <CardBox>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <FormField label="Tanggal Notulen / Rapat">
          <div class="font-medium">{{ formatDate(item.entry_date) }}</div>
        </FormField>

        <FormField label="Agenda / Nama Kegiatan">
          <div class="font-medium">{{ item.title }}</div>
        </FormField>

        <FormField label="Pimpinan Rapat / Narasumber">
          <div class="font-medium">{{ item.person_name || '-' }}</div>
        </FormField>

        <FormField label="Tempat / Instansi">
          <div class="font-medium">{{ item.institution || '-' }}</div>
        </FormField>

        <FormField label="Uraian / Hasil Rapat" class="md:col-span-2">
      ...
      <div class="flex items-center justify-end gap-2">
        <Link
          :href="`/kecamatan/simulasi/buku-notulen/${item.id}/edit`"
