...
const confirmDelete = () => {
  if (itemToDelete.value) {
    router.delete(`/kecamatan/simulasi/buku-notulen/${itemToDelete.value.id}`, {
      onSuccess: () => {
        isDeleteModalActive.value = false
        itemToDelete.value = null
      },
    ...
    <CardBox class="mb-6" has-table>
      <div class="p-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
          <Link
            v-if="canCreate"
            href="/kecamatan/simulasi/buku-notulen/create"
              ...
              <td class="px-3 py-3">
                <BaseButtons type="justify-center" no-wrap>
                  <BaseButton
                    ...
                    :icon="mdiEye"
                    small
                    :href="`/kecamatan/simulasi/buku-notulen/${item.id}`"
                    title="Detail"
                  />
                  <BaseButton
                    v-if="canUpdate"
