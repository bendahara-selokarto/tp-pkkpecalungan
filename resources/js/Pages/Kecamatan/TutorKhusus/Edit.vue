...
const form = useForm({
  jenis_tutor: props.tutorKhusus.jenis_tutor ?? jenisTutorOptions[0]?.value ?? '',
  jumlah_tutor: props.tutorKhusus.jumlah_tutor ?? '',
  keterangan: props.tutorKhusus.keterangan ?? '',
})

const submit = () => {
  form.put(`/kecamatan/tutor-khusus/${props.tutorKhusus.id}`)
}
</script>

<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountGroup" title="Edit Data Tutor Khusus Kecamatan" main />
