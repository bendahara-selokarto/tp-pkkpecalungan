...
const updatePerPage = (event) => {
  ...
  router.get('/kecamatan/tutor-khusus', {
    ...props.filters,
    page: 1,
...
const deleteConfirmationMessage = 'Apakah Anda yakin ingin menghapus data tutor khusus ini?'
...
const confirmDelete = () => {
  ...
  router.delete(`/kecamatan/tutor-khusus/${deletingId.value}`, {
    onFinish: () => {
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton :icon="mdiAccountGroup" title="Data Tutor Khusus Kecamatan" main />

    <CardBox>
      <div class="mb-4 flex items-center justify-between gap-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Daftar Data Tutor Khusus</h3>
        <Link
          href="/kecamatan/tutor-khusus/create"
        ...
        <table class="w-full min-w-[720px] text-sm">
          <thead class="border-b border-gray-200 dark:border-slate-700">
            <tr class="text-left text-gray-600 dark:text-gray-300">
              ...
              <th class="px-3 py-3 font-semibold">Jenis Tutor</th>
              <th class="px-3 py-3 font-semibold text-center">Jumlah Tutor</th>
          ...
          <tbody>
            <tr
              v-for="(item, index) in props.tutorKhususItems.data"
              :key="item.id"
              class="border-b border-gray-100 align-top dark:border-slate-800"
            >
              <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-300">{{ index + 1 }}</td>
              <td class="px-3 py-3 text-gray-700 dark:text-gray-300">{{ jenisTutorLabel(item.jenis_tutor) }}</td>
              <td class="px-3 py-3 text-center text-gray-700 dark:text-gray-300">{{ item.jumlah_tutor }}</td>
              ...
              <td class="px-3 py-3">
                <div class="flex items-center gap-2">
                  <Link
                    :href="`/kecamatan/tutor-khusus/${item.id}`"
                  ...
                  <Link
                    :href="`/kecamatan/tutor-khusus/${item.id}/edit`"
      ...
      <PaginationBar
        :links="props.tutorKhususItems.links"
        :from="props.tutorKhususItems.from"
        :to="props.tutorKhususItems.to"
        :total="props.tutorKhususItems.total"
      />
    </CardBox>
