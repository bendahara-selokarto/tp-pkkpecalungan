...
const initialEntries = Array.isArray(props.report.manual_entries) && props.report.manual_entries.length > 0
  ? props.report.manual_entries.map((entry) => ({
    bidang: entry.bidang || defaultBidang,
...
const form = useForm({
  ...
  tahun_laporan: props.report.tahun_laporan || new Date().getFullYear(),
  pendahuluan: props.report.pendahuluan || '',
  keberhasilan: props.report.keberhasilan || '',
  hambatan: props.report.hambatan || '',
  kesimpulan: props.report.kesimpulan || '',
  penutup: props.report.penutup || '',
  disusun_oleh: props.report.disusun_oleh || props.defaultCompiledBy || '',
  jabatan_penanda_tangan: props.report.jabatan_penanda_tangan || props.defaultSignerRole || '',
  nama_penanda_tangan: props.report.nama_penanda_tangan || '',
  manual_entries: initialEntries,
})
...
<template>
  <SectionMain>
    <SectionTitleLineWithButton
      ...
      :title="`Edit Laporan Tahunan ${scopeLabel}`"
      main
    />
      ...
      <form class="space-y-5" @submit.prevent="submit">
        ...
        <div class="rounded-md border border-gray-200 p-4 dark:border-slate-700">
          ...
          <p class="mb-3 text-xs text-gray-500 dark:text-gray-400">
            Bagian ini hanya untuk data pelengkap. Data utama tetap diambil otomatis dari modul aplikasi sesuai tahun laporan.
          </p>
