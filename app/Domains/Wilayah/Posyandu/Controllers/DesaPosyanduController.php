...
class DesaPosyanduController extends Controller
{
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('Desa/Posyandu/Create');
    }

    public function store(StorePosyanduRequest $request): RedirectResponse
    {
        $this->authorize('create', Posyandu::class);
        $this->createScopedPosyanduAction->execute($request->validated(), ScopeLevel::DESA->value);

        return redirect()->route('desa.posyandu.index')->with('success', 'Data posyandu berhasil dibuat');
    }

    public function show(int $id): Response
    {
        $posyandu = $this->getScopedPosyanduUseCase->execute($id, ScopeLevel::DESA->value);
        $this->authorize('view', $posyandu);

        return Inertia::render('Desa/Posyandu/Show', [
            'posyandu' => [
                'id' => $posyandu->id,
    ...
    public function edit(int $id): Response
    {
        ...
        return Inertia::render('Desa/Posyandu/Edit', [
            'posyandu' => [
                'id' => $posyandu->id,
                'nama_posyandu' => $posyandu->nama_posyandu,
                'nama_pengelola' => $posyandu->nama_pengelola,
    ...
    public function update(UpdatePosyanduRequest $request, int $id): RedirectResponse
    {
        ...
        return redirect()->route('desa.posyandu.index')->with('success', 'Data posyandu berhasil diperbarui');
    }
