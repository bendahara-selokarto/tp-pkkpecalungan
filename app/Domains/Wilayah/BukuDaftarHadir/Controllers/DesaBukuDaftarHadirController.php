...
class DesaBukuDaftarHadirController extends Controller
{
    ...
    public function index(ListBukuDaftarHadirRequest $request): Response
    {
        ...
        return Inertia::render('Desa/BukuDaftarHadir/Index', [
            'items' => $items,
            'pagination' => [
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('Desa/BukuDaftarHadir/Create', [
            'activityOptions' => $this->activityOptions('desa'),
        ]);
    }
    ...
    {
        $this->authorize('create', BukuDaftarHadir::class);
        $this->createScopedBukuDaftarHadirAction->execute($request->validated(), 'desa');

        return redirect()
            ->route('desa.buku-daftar-hadir.index')
            ->with('success', 'Buku Daftar Hadir berhasil dibuat');
    }

    public function show(int $id): Response
    {
        $item = $this->getScopedBukuDaftarHadirUseCase->execute($id, 'desa');
        $this->authorize('view', $item);

        return Inertia::render('Desa/BukuDaftarHadir/Show', [
            'item' => [
                'id' => $item->id,
                'attendance_date' => $this->formatDateForPayload($item->attendance_date),
                'activity_id' => $item->activity_id,
    ...
    public function edit(int $id): Response
    {
        ...
        return Inertia::render('Desa/BukuDaftarHadir/Edit', [
            'item' => [
                'id' => $item->id,
                'attendance_date' => $this->formatDateForPayload($item->attendance_date),
                'activity_id' => $item->activity_id,
