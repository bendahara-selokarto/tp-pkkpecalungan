...
class DesaBukuNotulenRapatController extends Controller
{
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('Desa/BukuNotulenRapat/Create');
    }

    public function store(StoreBukuNotulenRapatRequest $request): RedirectResponse
    {
        $this->authorize('create', BukuNotulenRapat::class);
        $this->createScopedBukuNotulenRapatAction->execute($request->validated(), 'desa');

        return redirect()
            ->route('desa.buku-notulen-rapat.index')
            ->with('success', 'Buku Notulen Rapat berhasil dibuat');
    }

    public function show(int $id): Response
    {
        $item = $this->getScopedBukuNotulenRapatUseCase->execute($id, 'desa');
        $this->authorize('view', $item);

        return Inertia::render('Desa/BukuNotulenRapat/Show', [
            'item' => [
                'id' => $item->id,
                'entry_date' => $this->formatDateForPayload($item->entry_date),
                'title' => $item->title,
    ...
    public function edit(int $id): Response
    {
        ...
        return Inertia::render('Desa/BukuNotulenRapat/Edit', [
            'item' => [
                'id' => $item->id,
                'entry_date' => $this->formatDateForPayload($item->entry_date),
                'title' => $item->title,
