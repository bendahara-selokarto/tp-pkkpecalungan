...
class DesaBukuTamuController extends Controller
{
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('Desa/BukuTamu/Create');
    }

    public function store(StoreBukuTamuRequest $request): RedirectResponse
    {
        $this->authorize('create', BukuTamu::class);
        $this->createScopedBukuTamuAction->execute($request->validated(), 'desa');

        return redirect()
            ->route('desa.buku-tamu.index')
            ->with('success', 'Buku Tamu berhasil dibuat');
    }

    public function show(int $id): Response
    {
        $item = $this->getScopedBukuTamuUseCase->execute($id, 'desa');
        $this->authorize('view', $item);

        return Inertia::render('Desa/BukuTamu/Show', [
            'item' => [
                'id' => $item->id,
                'visit_date' => $this->formatDateForPayload($item->visit_date),
                'guest_name' => $item->guest_name,
    ...
    public function edit(int $id): Response
    {
        ...
        return Inertia::render('Desa/BukuTamu/Edit', [
            'item' => [
                'id' => $item->id,
                'visit_date' => $this->formatDateForPayload($item->visit_date),
                'guest_name' => $item->guest_name,
