...
class KecamatanKlipingController extends Controller
{
    ...
    public function index(ListKlipingRequest $request): Response
    {
        ...
        return Inertia::render('Kecamatan/Kliping/Index', [
            'items' => $items,
            'pagination' => [
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('Kecamatan/Kliping/Create');
    }

    public function store(StoreKlipingRequest $request): RedirectResponse
    {
        $this->authorize('create', Kliping::class);
        $this->createScopedKlipingAction->execute($request->validated(), 'kecamatan');

        return redirect()
            ->route('kecamatan.buku-kliping.index')
            ->with('success', 'Buku Kliping berhasil dibuat');
    }

    public function show(int $id): Response
    {
        $item = $this->getScopedKlipingUseCase->execute($id, 'kecamatan');
        $this->authorize('view', $item);

        return Inertia::render('Kecamatan/Kliping/Show', [
            'item' => [
                'id' => $item->id,
                'date' => $this->formatDateForPayload($item->date),
                'description' => $item->description,
    ...
    public function edit(int $id): Response
    {
        ...
        return Inertia::render('Kecamatan/Kliping/Edit', [
            'item' => [
                'id' => $item->id,
                'date' => $this->formatDateForPayload($item->date),
                'description' => $item->description,
