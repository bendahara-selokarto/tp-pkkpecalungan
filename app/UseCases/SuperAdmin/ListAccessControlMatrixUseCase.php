...
class ListAccessControlMatrixUseCase
{
    ...
    public function execute(array $filters = []): array
    {
        ...
        $rows = $this->buildRows($roleScopePairs);
        ...
        return [
            ...
            'summary' => [
                ...
                'hidden' => $filteredRows->where('mode', self::MODE_HIDDEN)->count(),
            ],
        ];
    }
     ...
     * @return Collection<int, array<string, mixed>>
     */
    private function buildRows(array $roleScopePairs): Collection
    {
        $rows = collect();
        ...
        foreach ($roleScopePairs as $pair) {
            ...
            $overrides = $this->roleMenuVisibilityService->roleModuleModeOverrides($role);
    ...
    private function resolveEffectiveMode(?string $groupMode, array $overrides, string $module): string
    {
        ...
        return is_string($override) ? $override : self::MODE_HIDDEN;
    }
